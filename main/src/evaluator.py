...
import numpy as np
import random
from financial_evaluation.lm_eval.utils import positional_deprecated, run_task_tests
import financial_evaluation.lm_eval.metrics
import financial_evaluation.lm_eval.models
import financial_evaluation.lm_eval.tasks
import financial_evaluation.lm_eval.base
from model_prompt import MODEL_PROMPT_MAP
from chatlm import ChatLM
import tasks as ta
import os

from financial_evaluation.lm_eval.decontamination.decontaminate import get_train_overlap
...
def simple_evaluate(
    ...
    :param model: Union[str, LM]
        Name of model or LM object, see financial_evaluation.lm_eval.models.get_model
    ...
    if isinstance(model, str):
        if model_args is None:
            model_args = ""
        if model.startswith("hf-causal"):
            print("debug args", model_args, batch_size, max_batch_size, device)
            lm = financial_evaluation.lm_eval.models.get_model(model).create_from_arg_string(
                model_args, {"batch_size": batch_size, "max_batch_size": max_batch_size, "device": device})
        else:
            args_dict = dict(item.split('=') for item in model_args.split(','))
            max_gen_toks = int(args_dict["max_gen_toks"]) # ori dict value is str
            temperature = float(args_dict["temperature"]) if "temperature" in args_dict else 0.0
            
            lm = ChatLM(model, url=url, max_gen_toks=max_gen_toks, temperature=temperature)
    else:
        assert isinstance(model, lm_eval.base.LM)
        lm = model

    if not no_cache:
        lm = financial_evaluation.lm_eval.base.CachingLM(
            lm,
            "lm_cache/"
...
def evaluate(
    ...
    eval_split = "valid" if eval_split == "validation" else eval_split
    if eval_split not in ("test", "valid"):
        raise ValueError(f"Unsupported eval_split: {eval_split}. Use 'test' or 'valid'.")
    ...
    for task_name, task in task_dict_items:
        ...
        if eval_split == "test" and task.has_test_docs():
            task_doc_func = task.test_docs
            task_set = "test"  # Required for caching in the decontamination
        elif eval_split == "valid" and task.has_validation_docs():
            task_set = "valid"  # Required for caching in the decontamination
            task_doc_func = task.validation_docs
        else:
            raise RuntimeError(f"Task {task_name} does not support eval_split={eval_split}")
        ...
        print(f"Task: {task_name}; split: {eval_split}; number of docs: {len(task_docs)}")
...
t in range(turn)], turn)
            ...
            for resp, req in zip(resps, filtered_reqs):
                ...
                if write_out:
                    ...
                    task = task_dict[task_name]
                    if isinstance(task, financial_evaluation.lm_eval.base.MultipleChoiceTask):
                        write_out_info[task_name][doc_id]["truth"] = doc["gold"]
                    elif isinstance(task, financial_evaluation.lm_eval.tasks.winogrande.Winogrande):
    ...
    for (task_name, metric), items in vals.items():
        ...
        stderr = financial_evaluation.lm_eval.metrics.stderr_for_metric(
    ...
    if write_out:
        ...
        output_name = f"{output_base_path}~{eval_split}~{task_name}~{timestamp}~write_out_info.json"
        ...
        # output_base_path = (
            ...
            # else pathlib.Path(".")
        # )
        output_base_path = os.path.join(output_path, "write_out") if output_path else "."
        try:
        ...
        for task_name, _ in task_dict_items:
            with open(
                os.path.join(output_base_path, output_name),
                "w",
                encoding="utf8",
            ) as fp:
...
def make_table(result_dict):
    ...
    try:
        ...
        rows = [["Task", "Version", "Metric", "Value", "", "Stderr"]]
        for k, dic in result_dict["results"].items():
            ...
            for m, v in dic.items():
                if m.endswith("_stderr"):
                    continue
                if m + "_stderr" in dic:
                    rows.append([k, version, m, "%.4f" % v, "+/-", "%.4f" % dic[m + "_stderr"]])
                else:
                    rows.append([k, version, m, "%.4f" % v, "", ""])
                k = ""
                version = ""
